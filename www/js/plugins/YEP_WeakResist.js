...
Game_Action.prototype.apply = function(target)
{
    ...
    result.weakPopupNeeded = false;
    result.resistPopupNeeded = false;
    var rateResult = this.calcElementRate(target);
    if (rateResult > weakResult)
    {
        result.weakPopupNeeded = true;
    }
    if (rateResult < resistResult)
    {
        result.resistPopupNeeded = true;
    }
}
...
{
    return this._result.weakPopupNeeded;
}

Game_Battler.prototype.resultResist = function()
{
    return this._result.resistPopupNeeded;
}
...
Sprite_Battler.prototype.setupDamagePopup = function()
{
    if (this._battler.isDamagePopupRequested())
    {
        if (this._battler.isSpriteVisible())
        {
            ...
            console.log(this._battler._result);
            if(this._battler.resultWeak())
            {
                sprite.setWeak();
            }
            else if(this._battler.resultResist())
            {
                sprite.setResist();
            }
...
Sprite_Damage.prototype.updateChild = function(sprite) {
    if (sprite.talBaseY == null)
    sprite.talBaseY = 0;
    sprite.dy += 0.5;
    sprite.ry += sprite.dy;
    if (sprite.ry >= sprite.talBaseY) {
        sprite.ry = sprite.talBaseY;
        sprite.dy *= -0.6;
    }
